...
class Temperatures1
{
	public static void main (String[] args)
	{
		...
		// add code here
		for (int week = 1; week <= nofWeeks; week++) {
            minT[week] = t[week][1]; // Anta att första värdet är minst
            maxT[week] = t[week][1]; // Anta att första värdet är störst
            sumT[week] = 0;
			
			for (int i = 1; i <= nofMeasurementsPerWeek; i++) { 
            	minT[week] = Math.min(minT[week], t[week][i]); //Jämför minT[week] och t[week][input]
                maxT[week] = Math.max(maxT[week], t[week][i]); //Returnerar den minsta/största variabeln.
                sumT[week] += t[week][i]; //Summera värdet för veckan
			} 
            
			avgT[week] = sumT[week] / nofMeasurementsPerWeek; // Delar sumT med antal mätningar för att få medelvärdet
		}
		...
		// add code here
        for (int week = 1; week <= nofWeeks; week++) { 
        	minTemp = Math.min(minTemp, minT[week]); //Jämför och visar minsta värde
            maxTemp = Math.max(maxTemp, maxT[week]);
            sumTemp += sumT[week]; //Summerar alla sumT
